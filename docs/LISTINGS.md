...
**Short — 89 chars.** For `server.json` (hard cap 100) and any field that truncates.

```
Search 3.3M+ IT jobs from 294K company career boards, track applications, tailor your CV.
```

**Medium — for awesome lists, one line after the link.**

```
Search 3.3M+ IT jobs from any MCP host — postings crawled straight from 294K company
career boards, with market-fit scoring, application tracking and CV tailoring.
```

**Full — for submission forms with a description box.**

```
freehire aggregates IT job postings straight from company career boards and other public
sources — 3.3M+ open roles from 294K companies — normalizes them into one schema, and tags
each with stack, seniority, region and work mode. The MCP server puts that behind tools any
MCP host can call: search and filter
...
```
Job search, application tracking, market fit and CV tailoring over the freehire API.
```

**Medium.**

```
Task-shaped agent skills over the freehire CLI — job search, application tracking, market
fit, CV tailoring and mail triage — plus the /job-search, /market-fit, /tailor-cv,
/track-applications and /triage-inbox commands.
```
...
top of freehire, an open aggregator of IT postings pulled straight from company career
boards.
...
```
The open-source job search engine that reads company career boards directly.
```

**Short.**

```
freehire aggregates 3.3M+ IT jobs straight from 294K company career boards — no reposts,
no middlemen, no paywall. Search a normalized, deduplicated market by stack, seniority,
region and work mode. Free and open source, with a public API, a CLI and an MCP server
for agents.
```
...
```
Most job boards show you postings a recruiter chose to repost. freehire reads company
...
an agent: there is a documented HTTP API, a Go CLI, an MCP server for Claude Desktop and
Claude Code, and a custom GPT.
```
...
| Asset | State |
|---|---|
| OG image | `https://freehire.me/og.png` — rectangular |
| square icon | **missing.** Needed for `server.json` `icons` and most directory forms |
| screenshots | none prepared |

A listing ships without an icon rather than with a cropped one.
